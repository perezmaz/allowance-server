...
const { allowanceValidations } = require('../../validations');
...
const routeCreate = async (req, res) => {
  ...
  const { error } = allowanceValidations('create', data);
  if (error) {
...
const routeUpdate = async (req, res) => {
  ...
  const { error } = allowanceValidations('update', data);
  if (error) {
    sendValidationError(error, res);
  } else {
...
const routeFindAmount = async (req, res) => {
  const data = req.query;
  const token = req.headers.authorization.replace(/['"]+/g, '');
  const parent = decodeToken(token);

  const { error } = allowanceValidations('findAmount', data);
  if (error) {
    sendValidationError(error, res);
  } else {
    const response = await findAmount(data, parent._id);
    res.status(response.status).send(response);
  }
};
